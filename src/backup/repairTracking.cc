...
repairTracking::repairTracking(std::vector<Frame> *pRepairFrames, Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor):mrepairFrames(pRepairFrames),
    mState(OK), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mnLastRelocFrameId(0)//,mpKeyFrameDB(pKFDB),repairMap(pMap)
{
    repairMap = new Map();
    //repairMap = pMap;
	//mpKeyFrameDB = pKFDB;
...
void repairTracking::repairTrack()
{
    ...
    unique_lock<mutex> lock(repairMap->mMutexMapUpdate);
...
bool repairTracking::NeedNewKeyFrame()
{
    ...
    const int nKFs = repairMap->KeyFramesInMap();
...
void repairTracking::CreateNewKeyFrame()
{
    ...
    KeyFrame* pKF = new KeyFrame(mCurrentFrame,repairMap,mpKeyFrameDB);
...
void repairTracking::UpdateLocalMap()
{
    ...
    repairMap->SetReferenceMapPoints(mvpLocalMapPoints);
