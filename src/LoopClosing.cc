...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames()&& repairTracking::nRepairStop)
        {
        ...
        if(!mbStopGBA)
        {
            ...
            for(size_t i=0; i<vpMPs.size(); i++)
            {
                ...
                if(pMP->mnBAGlobalForKF==nLoopKF)
                {
                    ...
                    logCorrectMapPoints<<"If optimized by Global BA, just update"<<endl;
                    logCorrectMapPoints<<pMP->mnId<<" "<<pMP->mPosGBA.dims<<"ReferenceKeyFrame: "<<pMP->GetReferenceKeyFrame()->FrameSN<<endl;
                    //if(pMP->mPosGBA.dims>1)
                        //continue;
                ...
                else
                {
                    ...
                    logCorrectMapPoints<<"Update according to the correction of its reference keyframe"<<endl;
                    ...
                    logCorrectMapPoints<<pRefKF->mTcwBefGBA.dims<<" "<<pRefKF->FrameSN<<pRefKF->mTcwBefGBA<<endl;
                    if(!pRefKF->mTcwBefGBA.dims)
                        continue;
